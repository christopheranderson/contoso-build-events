...
import { type ApiError, type OrganizationCreate, OrganizationsService } from "@/client"
...
const AddOrganization = () => {
  ...
  const mutation = useMutation({
    mutationFn: (data: OrganizationCreate) =>
      OrganizationsService.createOrganizationRoute({ requestBody: data }),
    onSuccess: () => {
    ...
    >
      ...
      <DialogContent>
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <DialogBody>
            <Text mb={4}>Fill in the details to add a new organization.</Text>
            <VStack gap={4}>
              <Field
                required
                invalid={!!errors.name}
