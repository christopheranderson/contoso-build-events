...
from app.models import Organization, OrganizationCreate, OrganizationUpdate, OrganizationList

async def get_organization(db: AsyncSession, id: str) -> Optional[Organization]:
...
async def update_organization(db: AsyncSession, db_obj: Organization, obj_in: OrganizationUpdate) -> Organization:
...
async def list_organizations(db: AsyncSession, skip: int = 0, limit: int = 10) -> OrganizationList:
    result = await db.exec(select(Organization).offset(skip).limit(limit))
    organizations = result.all()
    count_result = await db.exec(select(sqlmodel_func.count()).select_from(Organization))
